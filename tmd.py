...
class TMD:
    ...
    def eval(self,x,Q2,kT,had,dist,irep):

        self.parman.set_new_params(self.par[irep],initial=True)

        if  dist=='pdf' and had=='p':
            return conf['pdf'].get_tmd(x,Q2,kT,'p','pdf')  
        elif dist=='ff' and had=='pi+': 
            return conf['ffpi'].get_tmd(x,Q2,kT,'pi','ffpi')  
        elif dist=='sivers' and had=='p':
            return conf['sivers'].get_tmd(x,Q2,kT,'p','sivers')  
        elif dist=='transversity' and had=='p':
           return conf['transversity'].get_tmd(x,Q2,kT,'p','transversity')  
        elif dist=='collinspi' and had=='pi':
            return conf['collinspi'].get_tmd(x,Q2,kT,'pi','collinspi')  
        else:
